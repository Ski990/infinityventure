...
<h4 class="card-title">Active Member List</h4>
...
<tr>
<td>
<div align="left" style="margin-left:10px;"><a href="member-active-csv-download.php"><input type="button" value="Excel Download" class="btn" style="background:#009900;color:#FFFFFF;" /></a></div>
</td>
<td>
<div align="right" style="padding:10px;"><form name="frm1" method="post" action="member-active.php?act=search"><input type="text" name="search" id="search" class="form-control input-line input-medium" value="<?=$_REQUEST['search']?>" required placeholder="User ID, Name, Phone" style="width:170px;" />
</form></div>
</td>
</tr>
</table>
...
{
$where="WHERE (`userid` LIKE '%".trim($_POST['search'])."%' OR `name` LIKE '%".trim($_POST['search'])."%' OR `phone` LIKE '%".trim($_POST['search'])."%') AND `paystatus`='A' ORDER BY `id` DESC";
}else{
...
<td align="center" style="padding:2px;"><?php if($fetch['package']){?><?=getSettingsPackage($conn,$fetch['package'],'package')?><?php }else{?>---<?php }?></td>
