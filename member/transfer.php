...
$sqlc="SELECT * FROM `iv_withdrawal` WHERE `userid`='".getMember($conn,$_SESSION['mid'],'userid')."' AND `date`='".date('Y-m-d')."'";
...
$adminamt=(trim($_POST['amount']) * $admin)/100;

$payout=(trim($_POST['amount'])-($tdsamt+$adminamt+$imps));
...
$sql1="INSERT INTO `vd_withdrawal_imps` (`userid`,`request`,`tds`,`admin`,`imps`,`payout`,`rcstatus`,`errormsg`,`operatorid`,`tracker`,`actualname`,`date`) VALUES('".getMember($conn,$_SESSION['mid'],'userid')."','".mysqli_real_escape_string($conn,$_POST['amount'])."','".$tdsamt."','".$adminamt."','".$imps."','".$payout."','".$rcstatus."','".$errormsg."','".$txid."','".$beneficiaryid."','".$desc."','".date('Y-m-d')."')";
$res1=query($conn,$sql1);

$message="jolo order id: $txid <br/> detail: $desc <br/> jolo status: $rcstatus <br/> beneficiaryid: $beneficiaryid";
