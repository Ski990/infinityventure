...
<div class="card-title"><a href="javascript:history.go(-1)"><img src="images/back-page.png" height="16" width="20"> &nbsp; Platinum Package Upgrade</a>   </div>
</div>
</div>
...
<div>&nbsp;</div>
<form class="form" action="platinum-package-upgrade-process.php" method="post">
...
<?php
$sqlt="SELECT * FROM `iv_member_platinum_package` WHERE `userid`='".$userid."'";
$rest=query($conn,$sqlt);
$numt=numrows($rest);
if($numt>0)
{
$sqlst="SELECT * FROM `iv_settings_platinum_package` WHERE `amount`>(SELECT `amount` FROM `iv_member_platinum_package` WHERE `userid`='".$userid."' ORDER BY `amount` DESC LIMIT 1 ) ORDER BY `amount` LIMIT 1";
}else{
$sqlst="SELECT * FROM `iv_settings_platinum_package` ORDER BY `amount` LIMIT 1";
}
